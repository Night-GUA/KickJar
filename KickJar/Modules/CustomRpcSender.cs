...
public class CustomRpcSender
{
    ...
    public State CurrentState
    {
        ...
        set
        {
            ...
            else Main.Logger.LogWarning("CurrentStateはisUnsafeがtrueの時のみ上書きできます");
        }
    }
    ...
    public CustomRpcSender(string name, SendOption sendOption, bool isUnsafe)
    {
        ...
        onSendDelegate = () =>  Main.Logger.LogInfo($"{this.name}'s onSendDelegate =>");

        currentState = State.Ready;
        Main.Logger.LogInfo($"\"{name}\" is ready");
    }
    ...
    public CustomRpcSender StartMessage(int targetClientId = -1)
    {
        if (currentState != State.Ready)
        {
            ...
            if (isUnsafe)
            {
                Main.Logger.LogWarning(errorMsg);
            }
            else
            {
    ...
    public void SendMessage()
    {
        ...
        Main.Logger.LogInfo($"\"{name}\" is finished");
        stream.Recycle();
    }
